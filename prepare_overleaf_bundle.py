...
All experiments are executed on an edge-workstation testbed representative of high-performance robotic and industrial vision systems:
\begin{itemize}
    \item \textbf{Host Processor}: 13th Gen Intel Core i7-13620H (10 cores: 6 Performance, 4 Efficient, up to 4.90 GHz), 24 MB L3 cache, 11.54 GB system RAM.
    \item \textbf{Edge Accelerator}: NVIDIA GeForce RTX 4050 Laptop GPU (Ada Lovelace architecture, 2560 CUDA cores, 80 Tensor Cores, 6 GB GDDR6 VRAM, 96-bit bus width, 95W TGP, CUDA 12.1, Driver 550.54).
    \item \textbf{Software Stack}: Ubuntu 22.04.5 LTS on WSL2 (Linux kernel 6.6.36.6-microsoft-standard-WSL2), Python 3.13, PyTorch 2.6.0+cu124, ONNX Runtime 1.20.1, TensorRT 10.8.0.
\end{itemize}
...
\begin{subfigure}[b]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/pareto_quality_vs_latency.png}
    \caption{Quality vs. Latency Pareto Frontier}
    \label{fig:pareto}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/speedup_comparison.png}
    \caption{Relative Speedup vs. Baseline}
    \label{fig:speedup}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/tail_latency_p50_p95.png}
    \caption{Tail Latency Distribution ($p_{50}$ vs. $p_{95}$)}
...
Table~\ref{tab:latency} reports multi-session latency distributions ($p_{50}, p_{95}$), numerical divergence ($L_\infty$ and Cosine Distance against PyTorch FP32), memory footprint, and throughput.
...
As visualized in the Pareto frontier of Fig.~\ref{fig:pareto}, GPU acceleration delivers an order-of-magnitude latency advantage over CPU execution, achieving $0.83$ ms model latency ($1,201.2$ FPS) on YOLO Nano. In Fig.~\ref{fig:speedup}, we examine the speedup ratios across runtime engines. On GPU, ONNX Runtime and TensorRT deliver up to $4.3\times$ speedup over CPU baselines. However, on CPU backends, static INT8 PTQ incurs a modest latency regression ($4.54$ ms vs. $3.59$ ms for FP32) due to the quantization/dequantization (QDQ) operator overhead on lightweight convolutional architectures lacking hardware AVX-512 VNNI acceleration.
...
Table~\ref{tab:q_aware} and Fig.~\ref{fig:q_aware_pareto} present the task-level accuracy recovery achieved by Q-Aware NMS across floating-point and quantized models.
